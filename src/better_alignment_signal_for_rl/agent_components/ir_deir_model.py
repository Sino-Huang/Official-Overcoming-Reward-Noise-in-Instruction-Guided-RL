...
from better_alignment_signal_for_rl.agent_components.ir_base_model import IntrinsicRewardBaseModel
from better_alignment_signal_for_rl.agent_components.deir.algo.common_models.mlps import *
...
class DiscriminatorModel(IntrinsicRewardBaseModel):
    def __init__(
        ...
        assert not use_status_predictor, "Status predictor is not supported in this project."
    ...
    def _get_fake_obs(self, curr_obs, next_obs):
        ...
        obs_diff1 = th.abs(next_obs - random_obs1).sum((1, 2, 3)) # sum over the 1,2,3 dims and leave the batch dim
        obs_diff2 = th.abs(next_obs - random_obs2).sum((1, 2, 3))
        obs_diff1 = th.gt(obs_diff1, th.zeros_like(obs_diff1)).long().view(-1, 1, 1, 1) # expand dims
        obs_diff2 = th.gt(obs_diff2, th.zeros_like(obs_diff2)).long().view(-1, 1, 1, 1)
        obs_diff = th.logical_or(obs_diff1, obs_diff2).long().view(-1, 1, 1, 1) # logical or 

        # return `random_obs1` when `next_obs` differs from `random_obs1`, otherwise `random_obs2`
        # this mean in a very large chance we will have the fake observation that is different from the true observation
    ...
    def _init_obs_queue(self):
        ...
        self.obs_queue = np.zeros((self.dsc_obs_queue_len,) + self.obs_shape, dtype='float32')


    def _get_dsc_embeddings(self, curr_obs, next_obs, last_mems, device=None):
        # print curr_obs, next_obs dtype 
        if not isinstance(curr_obs, Tensor):
            curr_obs = obs_as_tensor(curr_obs, device)
            next_obs = obs_as_tensor(next_obs, device)

        # Get CNN embeddings
        curr_cnn_embs = self._get_cnn_embeddings(curr_obs) # shape (batch_size, cnn_features_dim) 
        ...
        if self.use_model_rnn:
            ...
            curr_rnn_embs = th.squeeze(curr_mems[:, -1, :]) # it means the last hidden state of the rnn
    ...
    def update_obs_queue(self, is_obs_queue_init, intrinsic_rewards, ir_mean, new_obs):
        """
        ...
        for env_id in range(new_obs.shape[0]):
            if not is_obs_queue_init or intrinsic_rewards[env_id] >= ir_mean: # not is_obs_queue_init means the queue is empty thus we always add the obs
                obs = new_obs[env_id]
                self._add_obs(obs)
    ...
    def get_intrinsic_rewards(self,
        ...
        for env_id in range(batch_size):
            ...
            new_embs = [curr_obs_emb, next_obs_emb] if obs_embs is None else [obs_embs, next_obs_emb] # obs_embs is a sequence of observation 
    ...
    def optimize(self, rollout_data, env_name):
        # ! rollout data should come from storage.get_data_loader's minibatch 
        # Prepare input data
        with th.no_grad():
            ...
            curr_obs = rollout_data['obs']
            next_obs = rollout_data['new_obs']

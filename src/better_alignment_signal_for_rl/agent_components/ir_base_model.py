...
from better_alignment_signal_for_rl.agent_components.deir.algo.common_models.gru_cell import CustomGRUCell
...
from better_alignment_signal_for_rl.agent_components.deir.utils.common_func import init_module_with_name
...
class IntrinsicRewardBaseModel(nn.Module):
    ...
    def _get_rnn_embeddings(self, hiddens: Optional[Tensor], inputs: Tensor, modules: List[nn.Module]):
        outputs = []
        for i, module in enumerate(modules): # modules are layers of GRU
            hidden_i = th.squeeze(hiddens[:, i, :]) # squeeze the hidden state dim 1, which is the sequence length
            output_i = module(inputs, hidden_i) # output can be seen as the updated hidden state, see line 131 in deir.py
            inputs = output_i
            outputs.append(output_i)
        outputs = th.stack(outputs, dim=1) # we maintain the hidden states of all layers, and the outputs can be seen as the updated hidden states, aka the memory
        return outputs


    def _get_cnn_embeddings(self, obs, module=None):
        # obs = preprocess_obs(obs, self.observation_space, normalize_images=self.normalize_images) # already preprocessed in other places
        if module is None:

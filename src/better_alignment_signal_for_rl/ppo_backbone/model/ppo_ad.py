...
class PPOADModel(PPOModel):
    """PPO model implementation with achievement distillation, responsible for defining the neural network architecture and forward pass.
    """
    def __init__(
        self,
    ...
    def forward(self, obs: th.Tensor, states: th.Tensor, beta: th.Tensor = None, **kwargs) -> Dict[str, th.Tensor]:
        # Pass through encoder
        latents = self.encode(obs)

        # Concatenate latents and states
        # ! the state info will tell the agent about current achievement status, 
        # ! the state will not change until a new achievement is unlocked
        if self.use_memory:
        ...
        # Pass through heads
        pi_logits = self.pi_head(pi_latents, beta) # ! add the beta Boltzmann rationality coefficient beta
        # beta shape (env_num, 1)
    ...
    def film(self, latents: th.Tensor, actions: th.Tensor) -> th.Tensor:
        ...
        # fuse the action info into the latents
        num_actions = getattr(self.action_space, "n")
    ...
    def get_states(self, goal_obs: th.Tensor, goal_next_obs: th.Tensor) -> th.Tensor:
        ...
        batch_size = goal_obs.shape[0] # shape (batch_size, 3, 64, 64)
        zero_obs_conds = goal_obs.reshape(batch_size, -1) == 0 # check if the observation is all zeros 
    ...
    def compute_pred_losses( # calculate the intra trajectory loss 
        self,
        ...
        logits = th.stack([pos_logits, neg_logits], dim=-1) # shape: [b, 2], pos is at index 0
        logits = logits / self.temperature
        targets = th.zeros(len(logits)).to(logits.device).long() # shape: [b], 0 means leaning towards positive
    ...
    def compute_match_losses(
        ...
        neg_logits = th.einsum("bk,bk->b", anc_states, neg_states) # shape: [b]
        logits = th.stack([pos_logits, neg_logits], dim=-1) # shape: [b, 2], pos is at index 0
        logits = logits / self.temperature
        targets = th.zeros(len(logits)).to(logits.device).long() # shape: [b], 0 means leaning towards positive
        ...
        pi_dist = self.pi_head.kl_divergence(pi_logits, old_pi_logits).mean() # measure the KL divergence between the two distributions

...
class Buffer: # this buffer will mainly focus on the trajectory of getting an achievement
    ...
    def parse_segs(self):
        ...
        obs = th.cat([seg["obs"][:-1] for seg in self.segs], dim=0) # shape [aux_freq * 512(rollout_length), 8(nproc), 3, 64, 64]
        ...
        for p in range(nproc):
            ...
            # Get done steps
            done_conds_p = (masks_p == 0).squeeze(dim=-1) # shape [aux_freq * 512(rollout_length), ]
            done_steps_p = done_conds_p.nonzero().squeeze(dim=-1) # nonzero() returns the indices of the elements that are non-zero, squeeze() remove the dimension of size 1 
            done_steps_p = done_steps_p.tolist()
            done_steps_p = sorted(done_steps_p) # we get index of the last step of each trajectory

            for start, end in zip(done_steps_p[:-1], done_steps_p[1:]):
                # Get trajectory
                obs_t = obs_p[start:end] # this is the trajectory of the process p
                ...
                self.trajs.append(traj) # we are getting the trajectory of each process
    ...
    def get_goals(
        ...
        goal_steps_r = goal_steps_r + 1 # + 1 because successes[1:] will implicitly shift the index by 1, thus we need to shift it back

        # Get goal steps from successes
        goal_conds_s = (successes[1:] != successes[:-1]).any(dim=-1) # this will check new success
        goal_steps_s = goal_conds_s.nonzero().squeeze(dim=-1) # the index 
        goal_steps_s = goal_steps_s + 1 # + 1 because successes[1:] will implicitly shift the index by 1, thus we need to shift it back
        ...
        if len(goal_steps) == 0:
            ...
            goal_obs = obs[goal_steps - 1] # the obs of the goal step, this is before we take the action
            goal_next_obs = obs[goal_steps] # the next obs of the goal step, this is the actual goal observation 
    ...
    def get_next_goals( # ! this will give the next goal obs for every interval between the goal steps, eventually the shape[0] will be the same as the trajectory length
        self,
        ...
        for i, (start, end) in enumerate(zip(goal_steps[:-1], goal_steps[1:])):
            if i == len(goal_steps) - 2:
                ...
                next_goal_next_ob = th.zeros_like(obs[-1]).unsqueeze(dim=0) # next_goal_next_ob will be zero if there is no next goal
            else:
    ...
    def get_pred_data_loader(
        ...
        for i in th.randperm(ntraj): # randomly shuffle the trajectory
            # Get trajectory
            traj = self.trajs[i]
            obs = traj["obs"]
    ...
    def get_match_data_loader(
        ...
        for i in th.randperm(ntraj):
            ...
            for j in inds:
                ...
                T = entropic_partial_wasserstein(a, b, M, reg=0.05, numItermax=100) # get the optimal transport matrix, which matches the achievements in different trajectories
...
class PPOADAlgorithm(BaseAlgorithm):
    def __init__(
        ...
        # Buffer
        self.buffer = Buffer(maxlen=aux_freq) # it saves the data from the RolloutStorage
    ...
    def update(self, storage: RolloutStorage):
        """
        Update the PPO-AD algorithm by performing optimization steps.

        Args:
            storage (RolloutStorage): The storage object containing the rollout data.

        Returns:
            dict: A dictionary containing the training statistics including policy loss, value function loss, and entropy.
        ...
        self.buffer.insert(seg) # append the current whole storage to the buffer list
        # the buffer size is the aux_freq, so when the buffer is full, it will start to preprocess the data
        ...
        for _ in range(self.ppo_nepoch):
            ...
            for batch in data_loader: # minibatch = 8, thus the data_loader will yield 8 batches, and do gradient descent 8 times
                # Compute loss for policy and value models 
                ...
                loss.backward()
                clip_grad_norm_(self.model.parameters(), self.max_grad_norm) # PPO uses gradient clipping to prevent exploding gradient
                self.optimizer.step()
        ...
        if self.ppo_count % self.aux_freq == 0:
            ...
            self.buffer.preprocess_trajs() # we only get goal obs and goal next obs
            ...
            for _ in range(self.aux_nepoch):
                ...
                for batch in match_data_loader:
                    # Compute match loss for Auxiliary match process
                ...
                for batch in pred_data_loader:
                    # Compute pred loss for Auxiliary prediction process
